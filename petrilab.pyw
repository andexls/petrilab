...
# Interface de inserção de PN e simulação
...
class EditarEvento:
    def __init__(self, parent, evento):
        ...
        f1 = Frame(self.top); f1.pack(side=TOP)
        f2 = Frame(self.top); f2.pack(side=TOP)
        Label(f3, text='Rótulo: ').pack(side=LEFT)
        self.rot=StringVar()
...
class EditarLugar:
    def __init__(self, parent, lugar):
        ...
        Label(f3, text='Ações').pack(side=TOP)
...
class Programa(Tk):
    def __init__(self):
        ...
        #self.mladder.add_checkbutton(label="Utilizar rótulos", variable=self.uselabels,
        ...
        # Criação do Canvas e scrollbars
    ...
    def contato(self):
        r = askquestion('Informar Bug/Sugestões','Bugs e sugestões podem ser enviados por e-mail para petrilab10@gmail.com.\n\n'
                                                 'Alternativamente, é possível enviar um Ticket de suporte no site do projeto,'
                                                 'sendo necessário para isso um cadastro gratuito no Sourceforge. Deseja abrir'
    ...
    def editaracao(self, acao):
        while True:
            l = askstring('Editar ação', 'Rótulo', initialvalue=acao.text)
            bad=False
            for i in '!@#$%¨&*(),.;:':
                if i in l or l=='':
                    showinfo('Rótulo inválido','O rótulo possui caracteres inválidos ou está vazio.')
